...
import { PlusCircle, Trash2, X, LogOut, ChevronUp, ChevronDown, GripVertical } from "lucide-react";
    ...
    <div ref={setNodeRef} style={style} className="relative group">
      {isVideo ? (
        <video
          ...
          className="rounded-md w-full h-auto object-cover aspect-video border-2 border-blue-300 dark:border-blue-600"
          controls
        />
      ) : (
        <img
          ...
          <Card className="bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700">
            <CardContent className="pt-6">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-2 p-4 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                  <label className="block text-lg font-semibold text-slate-900 dark:text-slate-100">Project Page Banner (Carousel) - Images and Videos</label>
                  <p className="text-xs text-slate-500 dark:text-slate-400">Manage banner carousel images and videos. Drag items to reorder them, or use the arrow buttons.</p>

                  {existingBannerMedia.length > 0 && (
                    <>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mt-4">Current Banner Media</label>
                      <DndContext
                        sensors={sensors}
                  ...
                  {newBannerMedia.length > 0 && (
                    <>
                      <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mt-4">New Banner Media to Upload</label>
                      <DndContext
                        sensors={sensors}
                        collisionDetection={closestCenter}
                  ...
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">Add More Banner Images and Videos</label>
                  <Input
                    type="file"
                    multiple
                ...
                <div className="space-y-4 p-4 border border-slate-200 dark:border-slate-600 rounded-lg bg-slate-50 dark:bg-slate-700/50">
                  ...
                  {(sections || []).map((section, index) => (
                    <div key={section.id} className="p-4 border border-slate-300 dark:border-slate-500 rounded-md relative space-y-3 bg-white dark:bg-slate-800">
                      ...
                      <div className="space-y-2">
                        <label className="text-sm font-medium text-slate-700 dark:text-slate-300">Video</label>
                        <div className="relative w-full max-w-xs">
                          {section.video ? (
