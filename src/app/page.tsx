    ...
    <div className="min-h-screen bg-background text-foreground">
        ...
        <div className="grid md:grid-cols-3 gap-8 items-center">
          <div className="md:col-span-2">
            <div className="inline-flex items-center gap-2 rounded-xl bg-secondary px-3 py-1 text-sm mb-4">
              ...
              <span className="text-secondary-foreground">{content?.hero?.availability || 'Loading...'}</span>
            </div>
            {/* ⭐ UPDATED: Dynamic H1 */}
          ...
          <div className="md:justify-self-end w-full">
            <Card className="rounded-xl shadow-lg">
              ...
              <CardContent className="space-y-2 text-sm text-muted-foreground">
                ...
                <div className="flex items-center justify-start gap-4 pt-4 mt-4 border-t border-border">
                  {socials.map((s) => (
                    <a key={s.label} href={s.href} className="text-muted-foreground hover:text-foreground transition-colors" target="_blank" rel="noopener noreferrer">
      ...
      <section id="projects" className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-6">
                ...
                <Card className="p-0 rounded-lg hover:shadow-xl transition-shadow h-full flex flex-col overflow-hidden">
                  ...
                  <div className="flex flex-col flex-grow p-4">
                    ...
                    <CardContent className="p-0 pt-4 flex-grow">
                      <p className="text-foreground">{p.blurb}</p>
          ...
          }) : <p className="text-muted-foreground md:col-span-2">No featured projects have been selected yet.</p>}
        </div>
        <div className="mt-8">
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className={`sm:max-w-[425px] transition-colors duration-300 ${isSubmitted ? 'bg-emerald-50 dark:bg-emerald-900/20' : ''}`}>
          {!isSubmitted ? (
            <>
              <DialogHeader>
              ...
              <div className="space-y-4 py-4">
                <div>
                  <h4 className="font-semibold text-sm text-muted-foreground">Your Project Idea:</h4>
                  <p className="text-sm p-3 bg-muted rounded-md mt-1">{projectDescription}</p>
                </div>
